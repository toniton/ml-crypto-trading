...
class TestLoadConfigurationNode:
    def test_renders_catalog(self, vcs):
        node = LoadConfigurationNode(ConfigurationService(vcs))
...
class TestGenerateProposalNode:
    ...
    def test_first_generation_uses_goal_and_catalog(self, vcs):
        llm = FakeLlmAdapter([ConfigurationProposal(summary="s", changes=[])])
        node = GenerateProposalNode(llm, ConfigurationService(vcs))
        result = node({
            "user_prompt": "x",
            "request": self._route(),
    ...
    def test_goal_format_includes_target_asset(self, vcs):
        llm = FakeLlmAdapter([ConfigurationProposal(summary="s", changes=[])])
        node = GenerateProposalNode(llm, ConfigurationService(vcs))
        node({
            "user_prompt": "x",
    ...
    def test_regeneration_includes_previous_proposal_and_full_validation(self, vcs):
...
class TestValidateProposalNode:
    ...
    def test_valid_proposal_passes(self, vcs):
        node = ValidateProposalNode(ConfigurationService(vcs))
    ...
    def test_invalid_proposal_reports_errors(self, vcs):
        node = ValidateProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
            summary="s",
    ...
    def test_scoped_request_warns_on_global_change(self, vcs):
        node = ValidateProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
            summary="aggro",
            changes=[
    ...
    def test_scoped_request_no_warning_on_asset_path(self, vcs):
        node = ValidateProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
            summary="cadence",
...
class TestPresentProposalNode:
    def test_renders_structured_presentation(self, vcs):
        node = PresentProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
    ...
    def test_valid_validation_keeps_approval(self, vcs):
        node = PresentProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
            summary="less conservative",
            changes=[ConfigChange(path="assets.BTC_USD.consensus.buy", old_value=1.3, new_value=1.1, reason="more trades")],
    ...
    def test_invalid_validation_omits_approval_and_shows_errors(self, vcs):
        node = PresentProposalNode(ConfigurationService(vcs))
        proposal = ConfigurationProposal(
