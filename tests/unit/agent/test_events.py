...
class TestAIEvent:
    def test_to_dict_shape(self):
        event = AIEvent(type="node_started", message_id="abc123", payload={"node": "x"})
        data = event.to_dict()
    ...
    def test_block_event_carries_id(self):
        event = AIEvent(type="block", message_id="abc", id="b3", payload=MarkdownBlock.from_text("hi"))
        data = event.to_json()
    ...
    def test_approval_payload_serializes_structured_actions(self):
        event = AIEvent(type="block", message_id="abc", id="b4", payload=ApprovalBlock.build())
    ...
    def test_non_block_events_omit_id(self):
        event = AIEvent(type="done", message_id="abc", payload={"kind": "general"})
        assert "id" not in event.to_dict()

    def test_agent_field_included_when_set(self):
        event = AIEvent(type="node_started", message_id="abc", agent="configuration", payload={"node": "x"})
        data = event.to_dict()
        assert data["agent"] == "configuration"

    def test_agent_field_omitted_when_empty(self):
        event = AIEvent(type="node_started", message_id="abc", payload={"node": "x"})
