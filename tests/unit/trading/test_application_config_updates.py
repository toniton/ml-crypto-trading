...
def _seed_config(vcs, assets=None, dynamic_quantity="min_qty", ref="HEAD", **overrides):
    config = {
        "assets": assets or [],
...
class TestApplyConfigUpdate:
    def test_hands_checked_out_config_to_engine(self, vcs):
        app = _make_app(vcs)
        commit = _seed_config(vcs, assets=[_asset({"buy": 2.0, "sell": 0.8})], dynamic_quantity="equity * 0.1")

        app._apply_config_update(commit.hash)

        applied = app._trading_engine.configs[-1]
        assert applied.assets[0].consensus.buy == 2.0
        assert applied.assets[0].consensus.sell == 0.8
...
class TestOnRefChange:
    def test_ignores_events_for_other_refs(self, vcs):
        app = _make_app(vcs)
        commit = _seed_config(vcs, assets=[_asset({"buy": 2.0, "sell": 0.8})], ref="production")
...
class TestTradingExecutorUpdateConfig:
    def test_applies_consensus_and_dynamic_quantity(self):
        ...
        assert executor.consensus_manager.factor_for("BTC_USD").buy == 2.0
        assert executor.consensus_manager.factor_for("BTC_USD").sell == 0.8
    ...
    def test_update_config_is_idempotent_for_factors(self):
        executor = _make_executor()
        config = TradingConfig.model_validate(
            {"assets": [_asset({"buy": 2.0, "sell": 0.8})], "dynamic_quantity": "min_qty"}
        )
