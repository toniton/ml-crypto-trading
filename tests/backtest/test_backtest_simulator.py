...
def _make_engine(tmp_path, ticker_symbol: str = "BTC_USD") -> tuple[BacktestSimulator, BacktestEventBus, Asset]:
    ...
    timestamps = [dp.timestamp for dp in loader.load(ticker_symbol)]
    ...
    engine = BacktestSimulator(clock, loader, execution_engine, bus, strategy=lambda *_: None)
...
class TestBacktestSimulator:
    def test_run_emits_market_snapshot_and_tick_events(self, tmp_path):
        ...
        assert len(ticks) == 3
        assert [p.close for p in market_points] == [Decimal("100"), Decimal("101"), Decimal("102")]
