...
def _make_request(**overrides) -> BacktestRequest:
    kwargs = {
        ...
        "data_source": BacktestDataSourceRequest(path="/tmp/data"),
    }
...
class TestBacktestSession:
    def test_new_session_is_created(self):
        session = BacktestSession(ticker_symbol="BTC_USD", request=_make_request())

        assert session.ticker_symbol == "BTC_USD"
        assert session.status == BacktestSessionStatus.CREATED
        assert session.started_at is None
        assert session.completed_at is None

    def test_ids_are_unique(self):
        first = BacktestSession(ticker_symbol="BTC_USD", request=_make_request())
        second = BacktestSession(ticker_symbol="BTC_USD", request=_make_request())
...
class TestBacktestRequest:
    ...
    def test_request_is_immutable(self):
        ...
        with pytest.raises(Exception):
            request.ticker_symbol = "ETH_USD"  # type: ignore[misc]
